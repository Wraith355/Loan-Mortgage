...
try:
    ...
    joblib_source = "sklearn.externals"
except ImportError:
    try:
        # Try importing standalone joblib
        import joblib
        joblib_source = "standalone"
    except ImportError:
        try:
            # Last resort: try importing from sklearn directly
            from sklearn import joblib
            joblib_source = "sklearn"
        except ImportError as e:
            joblib_source = None
            error_message = str(e)

# Show joblib import status after page config
if joblib_source:
    st.success(f"Successfully imported joblib from {joblib_source}")
else:
    st.error(f"""
